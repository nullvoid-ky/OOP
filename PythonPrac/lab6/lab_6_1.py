...
class Atm:
    ...
    def insert_card(self, bank, card_id):
...
class Bank:
    ...
    def deposit(self, atm : Atm, account : Account, money : int):
    ...
    def withdraw(self, atm : Atm, account : Account, money : int):
        if money <= 0 or account.money < money:
            return "Error"
        account.money -= money
        atm.money += money
    ...
    def transfer(self, atm : Atm, old : Account, new : Account, money : int):
...
print("Test Case #1")
def testcase_1(bank, card_id):
    if (my_bank.atm_list[0].insert_card(bank, card_id) != None ):
        print(my_bank.atm_list[0].insert_card(bank, card_id).id, my_bank.atm_list[0].insert_card(bank, card_id).card.id, "Success")
    else:
        print("Error")
testcase_1(my_bank, "12345")
print()
